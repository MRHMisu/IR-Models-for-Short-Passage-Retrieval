...
/**
 * This class helps to remove stop words from a collection of word.
 */
...
public class StopWordRemover {
     ...
     *
     * @param wordList: A list of words derived from a collection.
     * @return : A list of words after removing the stop word.
     */
    public static List<String> removeStopWord(List<String> wordList) {

        wordList.removeAll(ENG_STOP_WORDS_SET);

        return wordList;
    }

    /**
     * Check whether a word is stop word or not.
     *
     * @param word :The word that is checked whether it is stop word or not.
     * @return : true if the given word is stop word otherwise false.
     */
    private static boolean isEnglishStopWord(String word) {
        return ENG_STOP_WORDS_SET.contains(word.toLowerCase());
    }

    static {
        String[] EngStopWord = {"a", "a's", "able", "about", "above", "according", "accordingly", "across", "actually",
        ...
        for (int i = 0; i < EngStopWord.length; i++) {
            ...
            word = WordStemmer.getStemmedWord(word);
